...
def preprocess_image(image_path: str) -> np.ndarray:
    """Предобработка фото для лучшего распознавания"""
    ...
    # Адаптивный порог — помогает при разном освещении
...
def recognize_vin_on_photo(image_path: str) -> str | None:
    """Распознаёт VIN-код на фотографии через Tesseract OCR"""
    try:
        # Пробуем сначала оригинал
        img = Image.open(image_path)
        text = pytesseract.image_to_string(img, config='--psm 11 -c tessedit_char_whitelist=ABCDEFGHJKLMNPRSTUVWXYZ0123456789')
        text_clean = text.upper().replace(" ", "").replace("\n", "")
        matches = VIN_PATTERN.findall(text_clean)
        if matches:
            return matches[0]

        # Если не нашли — пробуем с предобработкой
        processed = preprocess_image(image_path)
        pil_img = Image.fromarray(processed)
        text2 = pytesseract.image_to_string(pil_img, config='--psm 11 -c tessedit_char_whitelist=ABCDEFGHJKLMNPRSTUVWXYZ0123456789')
